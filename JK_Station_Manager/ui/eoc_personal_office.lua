...
local function init()
    ...
    if integration.menu and type(integration.menu.registerCallback) == "function" then
        ...
        DebugError("[JKEOC][B144][PERSONAL_OFFICE] callback=REGISTERED attempts=" .. tostring(integration.attempts + 1))
        return
    end
    ...
    if integration.attempts < integration.maxAttempts and type(Helper.addDelayedOneTimeCallbackOnUpdate) == "function" then
        ...
        DebugError("[JKEOC][B144][LUA_ERROR] Personal Office callback unavailable after retries=" .. tostring(integration.attempts))
    end
end
