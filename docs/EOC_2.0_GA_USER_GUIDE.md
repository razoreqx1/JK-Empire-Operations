...
## The EOC 2.0 decision pattern

Major operational pages now follow the same player-facing sequence:

1. **EOC conclusion** — what EOC believes is happening.
2. **Do this next** — one specific player action or an instruction to continue monitoring.
3. **Supporting evidence** — optional details for players who want to inspect the reasoning.
4. **Verify result** — a fresh analysis reports RESOLVED, IMPROVING, UNCHANGED, or WORSENING.

The selected station and case remain visible while moving between pages. Navigation controls remain available. Actions that submit work remain blacked out until EOC returns a terminal result.

### Reading case evidence

Case values are snapshots from the last EOC analysis. They can differ from the live vanilla station screen until verification runs. PASS results are supporting evidence, not instructions to add money, ships, storage, or production. EOC highlights only the first failed or unknown prerequisite as the current blocker.

For import-only cases, local production is **not applicable**. If all reported prerequisites pass or do not apply, observe one operating or delivery cycle and then run Verify Result. Do not add infrastructure unless the fresh verification still identifies a blocker.